...
def private_inference_client(test_data, test_labels, input_shape, num_classes, batch_size=50):
    ...
    print("Test data shape {}".format(test_data[0:batch_size, :, :, :].shape))
...
if __name__ == "__main__":
    ...
    if config.experiment_name == 'mnist_fc' or config.experiment_name == 'mnist_conv':
        ...
        private_inference_client(test_data, test_labels, MNIST_INPUT_SHAPE, MNIST_NUM_CLASSES, config.batch_size)
    elif config.experiment_name == 'malaria_conv':
        test_data, test_labels = CommonUtils.load_data(MALARIA_TARGET_DATA_PATH_PREFIX)
        private_inference_client(test_data, test_labels, MALARIA_INPUT_SHAPE, MALARIA_NUM_CLASSES, config.batch_size)
    else:
